...
#define object_speed_modifier 0.2
...
float dist(vec3 p)//distance function
{
	...
	{
		vec3 np = p+bpos[i];
		float shape0 = max3(abs(np.x),abs(np.y),abs(np.z))-Scale;
		float shape1 = length(np)-Scale;
...
void main(void)
{
	...
	{
		bpos[i] = -4.0*vec3(Scale) + -Scale*vec3(
	...
	vec3 d = normalize( -znear*camBasisZ + P.x*camBasisX + P.y*camBasisY );
	vec3 p = camPos;
	...
	for (int i=0; i<render_steps; i++) //raymarch
	{
		dd = dist(p);
		p+=d * dd * 0.99;
		if (dd<0.1*Scale || dd>10.0*Scale) break;
	}

	if (dd<=10.0*Scale) //close enough
	...
	color -= hash(color.xy+ndc.xy)*.015;
	color -= length(ndc)*.1;
	color =cc(color,.5,.6);

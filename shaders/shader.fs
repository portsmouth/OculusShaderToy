...
float EPSILON_NRM	= 0.1 / 128.0;

// sea
const int ITER_GEOMETRY = 3;
const int ITER_FRAGMENT = 4;
const float SEA_HEIGHT = 2.9;
...
void main(void)
{
	...
	// tracing
	vec3 p;
	heightMapTracing(ray_origin,ray_direction,p);
	vec3 dist = p - ray_origin;
	vec3 n = getNormal(p, dot(dist,dist) * EPSILON_NRM);
	vec3 light = normalize(vec3(0.0,1.0,0.8));

	// color
	vec3 color = mix(
		getSkyColor(ray_direction),
		getSeaColor(p,n,light,ray_direction,dist),
		pow(smoothstep(0.0,-0.05,ray_direction.y),0.3));
